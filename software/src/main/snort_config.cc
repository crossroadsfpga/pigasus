...
#ifdef PIGASUS
std::string SnortConfig::sid_patterns_filename;
std::unordered_map<uint32_t, uint32_t> SnortConfig::sid_compressed_sid_map;
#endif
...
#ifdef PIGASUS
    void SnortConfig::populate_sid_compressed_sid_map()
    {
        std::string line;
        int sid, compressed_sid = 1;  // compressed SIDs start at 1

        if (SnortConfig::sid_patterns_filename.size() == 0) {
            FatalError("You must provide a list with SIDs and patterns.\n");
        }

        std::ifstream f(SnortConfig::sid_patterns_filename);
        while(getline(f, line)) {
            line[line.find(':')] = '\0';
            int sid = atoi(line.c_str());
            SnortConfig::sid_compressed_sid_map[sid] = compressed_sid++;
        }
    }
#endif
